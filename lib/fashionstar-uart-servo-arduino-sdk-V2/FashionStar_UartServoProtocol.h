...
// 封包的长度
#define PACKAGE_BUFFER_SIZE 100
// 返回的响应数据包最长的长度
...
#define FSUS_CMD_SERVO_MONITOR       22 //数据监控
#define FSUS_CMD_SET_ORIGIN_POINT  23 // 设置原点
#define FSUS_CMD_STOP_ON_CONTROL_MODE  24 // 控制模式停止指令
#define FSUS_CMD_SYNC_COMMAND  25 // 同步指令
#define FSUS_CMD_BEGIN_ASYNC  18 // 开始异步命令
#define FSUS_CMD_END_ASYNC  19 // 开始异步命令
#define FSUS_CMD_RESET_MULTITURN_ANGLE  17 // 重设多圈角度
// FSUS状态码
...
typedef struct
{
	...
	uint16_t interval;              //单圈时间间隔
	uint32_t interval_multiturn;    //多圈时间间隔
	uint16_t t_acc;                 //加速时间
	uint16_t t_dec;                 //减速时间
	uint16_t power;                 //功率
}FSUS_sync_servo;
...
struct ServoMonitorData {
    ...
    float turns;
    bool isValid;  // 添加标识数据是否有效的字段
    FSUS_SERVO_ID_T servoId;  // 如果需要用到舵机ID
};
